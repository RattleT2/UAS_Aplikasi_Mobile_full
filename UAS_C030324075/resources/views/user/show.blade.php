...
@section('title', 'Detail Tiket - WebSpace Support')
...
async function loadTicket() {
    const result = await api('GET', '/inquiries/' + ticketId);
    if (!result || !result.ok) {
        document.getElementById('ticketDetail').innerHTML = '<div class="p-12 text-center text-red-500">Tiket tidak ditemukan.</div>';
        return;
    }
    ...
    document.getElementById('ticketDetail').innerHTML = `
        <a href="/dashboard" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-indigo-600 mb-4 transition-colors">
